...
trainer.load("294")  # Load the latest checkpoint
...
def video_tensor_to_gif_(tensor):
...
def quality_assessment_fn(x):
    ...
    """
    x_proc = x[0]
    x_in = x_proc.requires_grad_(True)
...
def p_sample(x, t, cond=None, cond_scale=1.0, clip_denoised=True):
    ...
    model_mean, _, model_log_variance = diffusion.p_mean_variance(x=x, t=t_tensor, clip_denoised=clip_denoised, cond=cond, cond_scale=cond_scale)
    ...
    temporary_unnorm_image = adjusted_mean + nonzero_mask * (0.5 * model_log_variance).exp() * noise
    
    return unnormalize_img(temporary_unnorm_image)
...
def generate_quality_guided_gif(input_gif):
    ...
    while current_quality < quality_threshold and t < diffusion.num_timesteps:
        # Perform the guided diffusion sampling step
        print("present t is -- " + str(t))
        b = input_tensor.shape[0]
        output = p_sample(input_tensor, torch.full((b,), t, device=device, dtype=torch.long), cond=None, cond_scale=1.0, clip_denoised=True)

        # Assess the quality of the current output
        temp_gif_path = "temporary_folder_save_gif/temp_output_"+str(t)+".gif"
        video_tensor_to_gif(output[0], temp_gif_path)
    ...
    final_output = video_tensor_to_gif(input_tensor.cpu(), "output_quality_guided.gif")
    return final_output

# Example usage
input_gif_path = 'test_sampling/759.gif'
